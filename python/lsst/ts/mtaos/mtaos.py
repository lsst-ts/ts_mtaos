...
class MTAOS(salobj.ConfigurableCsc):
    ...
    async def pubEvent_wavefrontError(self):
        ...
        for sensor_id, (zernike_indices, zernike_values) in self.model.get_wfe():
            ...
            for zernike_index, zernike_value, annular_zernike_coeff in zip(
                zernike_indices, zernike_values, annular_zernike_coeffs
            ):
                await self.evt_wavefrontError.set_write(
                    sensorId=sensor_id,
                    nollZernikeIndices=zernike_index,
                    nollZernikeValues=zernike_value,
                    annularZernikeCoeff=annular_zernike_coeff,
                    force_output=True,
                )
